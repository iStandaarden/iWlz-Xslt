...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:import href="../../../common/2_0/basis.xsl" />
		...
		<xsl:choose>
			...
			<xsl:when test="$pRule='CS015'">BerichtSubversie vullen met 2.</xsl:when>
			...
			<xsl:when test="$pRule='CS062'">4 (In zorg) vullen</xsl:when>
			...
			<xsl:when test="$pRule='CS108'">Vullen met een waarde die groter is dan, of gelijk is aan de Begindatum van de aangeduide periode én die niet groter is dan de Dagtekening van het bestand.</xsl:when>
			...
			<xsl:when test="$pRule='CS127'">2 (persoonsgebonden budget (PGB)) en 9 (verkeerde bed (VKB)) niet vullen.</xsl:when>
			...
			<xsl:when test="$pRule='CS131'">Vullen met een waarde die hoort bij de waarde van Leveringsstatus.</xsl:when>
			<xsl:when test="$pRule='CS132'">9 (Verkeerde bed (VKB)) niet vullen.</xsl:when>
			<xsl:when test="$pRule='CS133'">1 (Actief wachtend) niet vullen.</xsl:when>
			...
			<xsl:when test="$pRule='TR037'">Bij een Indicatie komt in ieder geval één zorgeenheid (GeindiceerdeFunctie of GeindiceerdZorgzwaartepakket) voor.</xsl:when>
			...
			<xsl:when test="$pRule='TR077'">Wanneer meer ToegewezenFuncties met dezelfde Functiecode en dezelfde Soort voorkomen, dan mogen deze ToegewezenFuncties maximaal één dag overlappen.</xsl:when>
			...
			<xsl:when test="$pRule='TR080'">De Begindatum van Periode moet precies één dag liggen na de Einddatum van Periode van een eerder correct ontvangen bestand van hetzelfde Zorgkantoor.</xsl:when>
			...
			<xsl:when test="$pRule='TR118'">MutatieZorgID moet overeenkomen met het GeleverdeZorgID van een actuele melding aanvang zorg CAK.</xsl:when>
			...
			<xsl:when test="$pRule='TR120'">Als Leveringsvorm in Aanvraag de waarde ‘4’ (Verblijf) bevat, mag er voor iedere combinatie van ToewijzingIngangsdatum en Soort maximaal één AanvraagInstelling aanwezig zijn waarvan Opname de waarde ‘1’ (Ja) bevat.</xsl:when>
			<xsl:when test="$pRule='TR121'">Een zorgaanbieder mag geen melding aanvang zorg sturen waarvan de begindatum ligt tussen de begindatum en de mutatiedatum van een eerder verstuurde melding einde zorg  (Mutatie met Mutatiecode 17, 19 of 20), die niet is verwijderd.</xsl:when>
			...
			<xsl:when test="$pRule='TR125'">Etmalen moet gelijk zijn aan Etmalen in de bijbehorende zorgtoewijzing.</xsl:when>
